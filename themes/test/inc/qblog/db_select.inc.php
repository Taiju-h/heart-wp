...
EOM;
	...
	//var_dump($chokuei);
			...
			$week = array("日", "月", "火", "水", "木", "金", "土");
			if(date("H")>4) $today = date("Y-m-d");
            else $today = date("Y-m-d", strtotime('yesterday'));
	
            $sql =  'SELECT Mkanteishi.name, Mtenpo.domain, Mtenpo.tenponame,  Mkanteishi.yomigana, Eworkdays.mtenpo_id, Mcast.mshutuen_id, Mcast.mkanteishi_id, Mtenpo.weboder, Mtenpo.weblistcnt, Mshutuen.img, Mshutuen.name as imgname , Mcast.mtenpo_id as cast_tenpo_id,  Eworkdays.mtime2_id, Eworkdays.mtime_id,
            Eworkdays.id as EworkdaysID
            FROM eworkdays  AS Eworkdays ';
            $sql .= ' LEFT JOIN mkanteishis AS Mkanteishi ON (Eworkdays.mkanteishi_id = Mkanteishi.id) ';
            $sql .= ' LEFT JOIN mtenpos AS Mtenpo ON (Eworkdays.mtenpo_id = Mtenpo.id) ';
            $sql .= ' 
                LEFT JOIN mcast AS Mcast  ON (Mcast.mkanteishi_id = Eworkdays.mkanteishi_id) ';
            $sql .= ' LEFT JOIN mshutuens AS Mshutuen ON (Mcast.mshutuen_id = Mshutuen.id) ';
            if(is_null($chokuei))
            $sql .= " WHERE   Mcast.mkanteishi_id <>5   AND Mcast.mdivision_id = 1  AND 
                (  
                    ( Eworkdays.workdate='{$today}' AND holiday_flg = 0 ) 
                    OR
                    ( Eworkdays.id<0 ) 
                )
                AND Mtenpo.chokuei = '1'  ";
			else $sql .=  " WHERE   Mcast.mdivision_id = 1";  

			
			if(date("H")>10) $sql .= " AND (Eworkdays.mtenpo_id != 51 OR Eworkdays.mtime_id > 1000
			OR Eworkdays.mtime2_id > 1000)			 ";

//Eworkdays.mtime_id開始時間,Eworkdays.mtime2_id終了時間がちゃんと現在の時間でOKであるものを表示する

//4時までは24に足す(最大27時になる)
$hm= date("H")*100+floor(date("i")*6/5);
if(date("H")<5) $hm+=2400;

//以下を含まない条件に追加
			$sql .= " AND NOT  ( 
				Eworkdays.mtenpo_id = 51 AND 
				Eworkdays.mtime_id  < " . $hm
				." AND 
				Eworkdays.mtime2_id < " . $hm
				." ) ";
		...
		$sql .= "    ORDER BY 
        CASE
            WHEN cast_tenpo_id IS NULL THEN 99
            WHEN cast_tenpo_id = 72 THEN 1 -- 上野広小路店
            WHEN cast_tenpo_id = 60 THEN 2 -- 浅草店
            WHEN cast_tenpo_id = 67 THEN 3 -- 浅草駅前店
            WHEN cast_tenpo_id = 53 THEN 4 -- 自由が丘店
            WHEN cast_tenpo_id = 66 THEN 5 -- 神田店
            WHEN cast_tenpo_id = 58 THEN 6 -- 御徒町店
            WHEN cast_tenpo_id = 73 THEN 7 -- 自由が丘南口店
            WHEN cast_tenpo_id = 83 THEN 8 -- すしや通り店
            WHEN cast_tenpo_id = 4 THEN 9 -- 伊勢佐木町店
            WHEN cast_tenpo_id = 38 THEN 10 -- 上野店
            WHEN cast_tenpo_id = 28 THEN 11 -- 大久保店
            WHEN cast_tenpo_id = 51 THEN 12 -- リモート占い館
            ELSE 99
        END,  

        CASE
            WHEN mtime_id IS NULL THEN 2
            WHEN mtime_id = 0 THEN 1
            ELSE 0
        END,  
		...
		//  echo $sql;
	//var_dump($sql . "chokuei = " . $chokuei);
            
            
			$data = mysqli_query($link, $sql);

            
            $fff = mysqli_query($link, "  select * from mtenpos where chokuei=1 ");
            $tenpoArray = array();// 空の配列を初期化
            ...
            while ($row = mysqli_fetch_assoc($fff)) $tenpoArray[$row['id']] = $row['tenponame'];
        ...
        $output .=  '<ul class="soothsayer_list">';
            $ready=[];
				...
				while ($mcast = mysqli_fetch_assoc($data)) {
						...
						$output .= "<!--";
						$output .= htmlspecialchars(var_export($mcast, true), ENT_QUOTES);
						$output .= "-->";



                    if($mcast["mtenpo_id"]==51 and $mcast['EworkdaysID'] >0 and
                       $mcast['mtime2_id'] < date("H")*100+date("i")*6/5 and
                      !in_array($mcast["mshutuen_id"], [1,3,17,14,22])) continue;
                    ...
                    // if($mcast["mtenpo_id"]==51)  continue;

                    if($mcast["mtenpo_id"]!=51){
                            if( $mcast['mtime2_id'] < date("H")*100+date("i")*6/5 ){
                                //退勤時間後は以下のステータス以外は表示しない
                                if($mcast["mtenpo_id"]!=$mcast['cast_tenpo_id']) continue;
                                if(!in_array($mcast["mshutuen_id"], [3,17,14,22] )) continue;
                                                   
                        }elseif(date("H")<5 ){ 
                        
                            if( $mcast['mtime2_id'] < (date("H")+24)*100+date("i")*6/5 ){
                                if(!in_array($mcast["mshutuen_id"], [3,17,14,22] )) continue;
                            }
                        }
                    }
                    ...
                    if(
                        ($mcast['mtime_id']==0 || $mcast['mtime2_id']==0 ||
                      $mcast['mtime_id']==null || $mcast['mtime2_id']==null)
                      &&  $mcast['cast_tenpo_id']!=51  ) continue;
                    ...
                    if($ready[$mcast["mkanteishi_id"]]++>0 ) continue;
                    //if($mcast["cast_tenpo_id"]==9999 && $mcast["mtenpo_id"]==51 ) continue;
                    //if($mcast["cast_tenpo_id"]==NULL && $mcast["mtenpo_id"]==51 ) continue;
                    
                    //if(==51) $mcast['tenponame']="リモート占い館";
                   if(date('H')>11 &&  $mcast["mtenpo_id"]!=51) $mcast['tenponame']= $tenpoArray[$mcast['cast_tenpo_id']];
                   else $mcast['tenponame']= $tenpoArray[$mcast['mtenpo_id']];

                    $mcast["mtenpo_id"]=$mcast['cast_tenpo_id'];
					...
					$output .= '<img src="' . USER_IMG3 . $wk_str . '.jpg?' . $time . '" alt="' . $alt . '">';

					$output .= "<h4 class='Mincho'>" . $mcast['name'] . "先生<span> " . $mcast['tenponame'] . "</span></h4>";
                    $strtime=heartftime($mcast['mtime_id'], $mcast['mtime2_id']);
					$output .= "<h4 class='Mincho'>" . $strtime. "</span></h4>";
                    
					$alt =  $mcast['imgname'];
					$output .= '<img src="' .USER_IMG2 .  $mcast['img'] . '"width="150" alt=' . $alt .'></a>';
                    //$output .= $mcast["mtenpo_id"] ." ". $mcast["mshutuen_id"];
...
//var_dump($Tfeedbacks );
	$time=time();	
		while ($row = mysqli_fetch_assoc($Tfeedbacks)) {
				...
				$output .= '<img src="' . $img3 . $wk_str . '.jpg?' . $time . '" alt="' . $row['Mname'] . '">';
...
//var_dump("AAA" . $id ."   " . $tel . "</br>");
		...
		if(is_null($pname) and (is_numeric($id)) ){
			$sq2 = 		$sq2 = "SELECT tenponame as name from mtenpos WHERE id = {$id}";
			$wk_name = "出演の先生たち";
		} else {
			switch ($kbn) {
				case '1':
...
// 指定の店舗順位をIDで定義
//$tenpo_order = [72, 67, 53, 60, 66, 58, 83, 73, 4, 38, 28];
$tenpo_order = [72, 60,67, 53,  66, 58,  73,83, 4, 38, 28, 51];

$order_ids = implode(',', $tenpo_order);

// FIELD関数を使って、Mcast.mtenpo_id を指定した順序で並べる
// リストにないID（リモート51や白山86など）は後回しにするため FIELD(...) = 0 を考慮
$sql .= " ORDER BY FIELD(Mcast.mtenpo_id, {$order_ids}) = 0, FIELD(Mcast.mtenpo_id, {$order_ids}),";
$sql .= '  Mshutuen.oder, Mkanteishi.mpost_id , Mkanteishi.mtankataimen_id DESC, Mkanteishi.id ';
	...
	while ($Mkanteishi = mysqli_fetch_assoc($Mkanteishis)) {
		if($new) {
			...
			if(isset($Day['workdate'])) {
				...
				$output .=  '出演予定' . mb_convert_kana($wkdate, 'N'); $output .=  '</br>';
...
$mds = mysqli_query($link, $sql);
	$old_kbn = 0;
		...
		if($id != 86)
				$output .= '<div class="sidebanner_hakusan"><a href="https://hakusan.uranai.heartf.com/">白山店</a></div>';
...
EOM;
	...
	function elog_list2($kbn = NULL, $id = null)  {
		...
		$link = mysqli_connect(SEVER, USER_ID, USER_PASS, USER_DB);	mysqli_set_charset(USER_CHATSET, $link );
		$output = NULL;
...
//	$output .=	'<h3 class="accordion__title js-accordion-title">鑑定料金別で探す</h3>	<ul class="judgment accordion__content">';
		...
		while ($data = mysqli_fetch_assoc($datas))
			$output .=	"<li><a href='index.php?{$data['pname']}' class='Mincho'>{$data['name']}</a></li>";
		$output .=  "</ul>";
...
function mkanteishi_imasugu(){
 
	//($id = null, $tel = NULL, $new = NULL) {
	$link = mysqli_connect(SEVER, USER_ID, USER_PASS, USER_DB);
	mysqli_set_charset(USER_CHATSET, $link );
	...
	$output = NULL;
	$sql = 'SELECT DISTINCT Mkanteishi.name, Mkanteishi.yomigana,Mkanteishi.pr_senjitsut, Mkanteishi.therapiest, Mkanteishi.mtankatel_id, Mkanteishi.mtankataimen_id,Mcast.mshutuen_id, Mcast.mtenpo_id, Mkanteishi.shasin,  Mkanteishi.id,  Mcast.mkanteishi_id   ';
	$sql .= ' FROM mkanteishis AS Mkanteishi,   mcast AS Mcast ';
	  $sql .= ' WHERE (Mkanteishi.delflg = 0) ';
	  $sql .= ' AND Mcast.mshutuen_id = 1 ';
	  $sql .= ' AND Mcast.mshutuen_id = 1 ';
	  $sql .= ' AND (Mcast.mdivision_id = 1) ';
	$sql .= ' AND Mkanteishi.id = Mcast.mkanteishi_id ';
	if(!is_null($id))  $sql .= 'AND Mshutsuenday.mtenpo_id = ' . $id;

	$sql .= ' ORDER BY   Mkanteishi.mpost_id , Mkanteishi.mtankataimen_id DESC, Mkanteishi.id ';
	$Mkanteishis = mysqli_query($link, $sql);

	$date = new DateTime('now');
	//echo $date->format('Y年m月d日 H時i分s秒');

	mysqli_data_seek($Mkanteishis, 0); // 念のため先頭に戻す

	$results = [];
	while ($row = mysqli_fetch_assoc($Mkanteishis)) {
		$id = (int)$row['id'];
		$sql_perm = "SELECT permissions, permissions51 FROM mcast WHERE mkanteishi_id = $id LIMIT 1";
		$permResult = mysqli_query($link, $sql_perm);
		$perm = mysqli_fetch_assoc($permResult);
	
		$row['permissions'] = $perm['permissions'] ?? '';
		$row['permissions51'] = $perm['permissions51'] ?? '';
...
$output .=  '<ul class="soothsayer_list">';
	...
	foreach ($results as $row) {
		$cast = [
			...
			'permissions51' => $row['permissions51'],
			'mshutuen_id' => $row['mshutuen_id'],
			'mtenpo_id' => $row['mtenpo_id'],
		];

	//	/.echo "s--".$row['mkanteishi_id'];
	
		$parsed = parsePermissionsStandalone($cast);
		$canSelect = $parsed['canSelect'];
		$imasugu_ids = [4, 6, 7, 8, 11, 13];
	
		foreach ($imasugu_ids as $id) {
			if (!empty($canSelect[$id]) && $canSelect[$id] == 1) {
				$filtered[] = $row;
				break;
			}
		}
	...
	//while ($Mkanteishi = mysqli_fetch_assoc($Mkanteishis)) {
	foreach ($filtered as $Mkanteishi) { 
		if($Mkanteishi['id']==5) continue;
			$sql_day = "SELECT workdate, weekday, mtimes1.name as name, mtimes2.name as name2 FROM `eworkdays` , mtimes as mtimes1, mtimes as mtimes2 WHERE mtime_id = mtimes1.id and mtime2_id = mtimes2.id and  holiday_flg = 0 and `mkanteishi_id` = " . $Mkanteishi['id'] ." AND `workdate` >=  '". $date->format('Y-m-d') . "'  ORDER by workdate LIMIT 2";
			$Days = mysqli_query($link, $sql_day);
...
//		
$output .='<li>';		$output .= '<a class="cards__item card" href="' . USER_URL . 'Mkanteishis/view/' . $Mkanteishi['id'] . '/51/">';
			$output .= '<div class="card__header">';
			$output .= '<div class="card__img">';
			 $output .= '<div class="card__detail">';
			// $output .= '<div class="card__title"><span>' . h($Mkanteishi['name']) . '先生</span>' . h($Mkanteishi['yomigana']) . '</div>';
		...
		$img3 = USER_IMG3;
			...
			$output .= "<h4 class='Mincho'>" . $Mkanteishi['name'] . "先生<span> " . $Mkanteishi['tenponame'] . "</span></h4>";
			$strtime=heartftime($Mkanteishi['mtime_id'], $Mkanteishi['mtime2_id']);
		
		// $wk_str = sprintf("%04d", $row['mkanteishi_id']);
		// 	$output .= "<img src='{$img3}{$wk_str}.jpg' . alt=' {$row['Mname']}'>";
				$output .= "<h4 class='Mincho'>" . $strtime. "</span></h4>";
			
			$alt =  $mcast['imgname'];
			$output .= '<img src="https://uranai.heartf.com/images/taiki.gif" width="150" alt="待機中"></a>';
			//$output .= $mcast["mtenpo_id"] ." ". $mcast["mshutuen_id"];
...
function parsePermissionsStandalone(array $mcast) {
    $allow = [];
    $deny = [];
    $allow51 = [];
    $deny51 = [];
    ...
    $conditionalClassIds = [4, 6, 7, 8, 11, 13];

    $mshutuenId = $mcast['mshutuen_id'] ?? null;
    $mtenpoId = $mcast['mtenpo_id'] ?? null;

    // 通常の permissions
    if (!empty($mcast['permissions'])) {
        foreach (explode(',', $mcast['permissions']) as $permission) {
            if (strpos($permission, ':') === false) continue;
            list($classId, $status) = explode(':', $permission);
            $classId = (int)$classId;
            $allow[$classId] = ($status === 'allow') ? 1 : (($status === 'deny') ? -1 : 0);
        }
    }

    // 店舗51用の permissions
    if (!empty($mcast['permissions51'])) {
        foreach (explode(',', $mcast['permissions51']) as $permission) {
            if (strpos($permission, ':') === false) continue;
            list($classId, $status) = explode(':', $permission);
            $classId = (int)$classId;
            $allow51[$classId] = ($status === 'allow') ? 1 : (($status === 'deny') ? -1 : 0);
        }
    }

    // 双方向補完
    $allow[4] = $allow51[4] ?? ($allow[4] ?? 0);
    $allow[6] = $allow51[6] ?? ($allow[6] ?? 0);
    $allow51[7] = $allow[7] ?? ($allow51[7] ?? 0);
    $allow51[8] = $allow[8] ?? ($allow51[8] ?? 0);

    $allow51[12] = $allow51[5] ?? 0;
    $allow51[13] = $allow51[6] ?? 0;
    $allow[12] = $allow[5] ?? 0;
    $allow[13] = $allow[6] ?? 0;

    // 判定ロジック
    foreach ($allow as $classId => $status) {
        if (in_array($classId, $conditionalClassIds, true)) {
            if ($mshutuenId == 1) {
                $canSelect[$classId] = ($status === 1 || (!empty($allow51[$classId]) && $allow51[$classId] === 1)) ? 1 : -1;
            } else {
                $canSelect[$classId] = -1;
            }
        } else {
            $allowStatus = $status === 1 || (!empty($allow51[$classId]) && $allow51[$classId] === 1);
            $denyStatus = $status === -1 && (!empty($allow51[$classId]) && $allow51[$classId] === -1);
    ...
    return compact('allow', 'allow51', 'canSelect');
}
